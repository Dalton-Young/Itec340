...
class DataSource {
  ...
  public void create_Account(ArrayList<String> query ) throws SQLException {
    try {
      CallableStatement stmt = conn.prepareCall("{call Create_Account(?, ?, ?, ?)}");
  ...
  public void remove_from_catalog(ArrayList<String> query ) throws SQLException {
    try {
      CallableStatement stmt = conn.prepareCall("{call remove_from_catalog(?,?,?)}");
      stmt.setInt(1,Integer.parseInt(query.get(1)));
      stmt.setInt(2,Integer.parseInt(query.get(2)));
      stmt.setString(3,"Message: ");
      stmt.execute();
  ...
  public String getBookInfo(ArrayList<String> query ) throws SQLException {
    ...
    String sqlquery = "select ISBN, title	, edition, pub_year, description, link, num_pages, primary_author from Books where " + searchID.substring(1,searchID.length()) + " = '" + searchBY +"'";
    ...
    try {
        ...
        while(rs.next()){
          ...
          bookInfo = bookInfo + "Publication Year: " + rs.getString("pub_year") +"\n";
          bookInfo = bookInfo + "Description: " + rs.getString("description") +"\n";
          bookInfo = bookInfo + "Link: " + rs.getString("link") +"\n";
          bookInfo = bookInfo + "Pages: " + rs.getString("num_pages") +"\n";
          bookInfo = bookInfo + "Author: " + rs.getString("primary_author") +"\n";
